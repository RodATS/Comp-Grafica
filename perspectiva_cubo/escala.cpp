...
void move(char ingresa, float& c_x, float& c_y, float& c_z, int &dim) {
    switch (ingresa)
    {
    case 'Q':
        c_x++;
...
int main() {
    ...
    Vertices A(1.0, 1.0, 1.0); Vertices B(-1.0, 1.0, 1.0); Vertices C(1.0, -1.0, 1.0); Vertices D(-1.0, -1.0, 1.0);
    Vertices E(1.0, 1.0, -1.0); Vertices F(-1.0, 1.0, -1.0); Vertices G(1.0, -1.0, -1.0); Vertices H(-1.0, -1.0, -1.0);

    Vertices A_O(1.0, 1.0, 1.0); Vertices B_O(-1.0, 1.0, 1.0); Vertices C_O(1.0, -1.0, 1.0); Vertices D_O(-1.0, -1.0, 1.0);
    Vertices E_O(1.0, 1.0, -1.0); Vertices F_O(-1.0, 1.0, -1.0); Vertices G_O(1.0, -1.0, -1.0); Vertices H_O(-1.0, -1.0, -1.0);
      ...
      */
      ...
      //PINTAR MATRIZ
        ...
        */
        ...
        //cout << "eje x:Q A, eje y:W S, eje z:E D crecer_dimension: R T cerrar:F\n";
       // cin >> movimiento;
        //dimension+=13;
     //   move(movimiento, c_x, c_y, c_z, dimension);
      if(dimension <= 78 && crecer==true) dimension += 13;
      if(dimension > 78) crecer = false;
      if(dimension > 13 && crecer==false) dimension -= 13;
      if(dimension == 13) crecer = true;
